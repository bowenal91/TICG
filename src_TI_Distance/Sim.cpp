...
void Sim::MCSim() {
    ...
    for (box.cycle=0;box.cycle<box.numCycles;box.cycle++) {
        ...
        if (box.cycle%box.TI_sample_interval == 0) {
            sample_TI_F();
            print_TI();
        }
    }
...
void Sim::read_input() {
  ...
  if (input!=NULL)
    { 	/******		Reading Input File		********/
      ...
      ret=fscanf(input,"%d",  &box.TI_sample_interval);               fgets(tt,2000,input);
      ret=fscanf(input,"%lf",  &box.TI_shift);               fgets(tt,2000,input);
...
void Sim::read_CNC_data() {
    int i;
    FILE *CNC_file = fopen("CNC.dat","r");
    fscanf(CNC_file,"%d",&box.numCNC);
    cnc = new CNC[box.numCNC];
    for (i=0;i<box.numCNC;i++) {
        fscanf(CNC_file,"%lf%lf",&cnc[i].rad,&cnc[i].L);
        fscanf(CNC_file,"%lf%lf%lf",&cnc[i].R[0],&cnc[i].R[1],&cnc[i].R[2]);
        fscanf(CNC_file,"%lf%lf%lf",&cnc[i].U[0],&cnc[i].U[1],&cnc[i].U[2]);
    }
...
void Sim::generate_matrices() {
    ...
    data = fopen("3Body.dat","r");
    for (i=0;i<box.numTypes;i++) {
        for (j=0;j<box.numTypes;j++) {
            for (k=0;k<box.numTypes;k++) {
                fscanf(data,"%lf",&ws[i][j][k]);
            }
        }
    }
    //printf("%f\n",ws[1][1][1]); 
...
void Sim::initialize_data() {
    ...
    double dummy2 = 2*sqrt(3.0)*PI*box.sigma*box.sigma;
    dummy = dummy*dummy*dummy;
    dummy2 = dummy2*dummy2*dummy2;
    box.prefact = 1.0/(dummy);
    box.prefact3 = 1.0/(dummy2);
...
//NEED TO FIX THIS TO DEAL WITH TRIPLET ENERGY AND PAIR ENERGY 
double Sim::calc_total_nonbond_energy() { //Loops through all grid cells and calculates pair interactions only for cells that are larger than current cell. This avoids double counting
    ...
    double E2 = 0.0;
    for (i=0;i<box.numBeads;i++) {
        E1 += calc_bead_pair_energy(i);
        E2 += calc_bead_triplet_energy(i);
    }   
...
double Sim::calc_bead_pair_energy(int i) {
    int ii,jj,kk,m,m2,x,y,z,j,k,iii,jjj,kkk;
    int xmin,xmax,ymin,ymax,zmin,zmax,xmin2,xmax2,ymin2,ymax2,zmin2,zmax2;
    m = beads[i].grid;
    x = grid[m].x;
    y = grid[m].y;
    z = grid[m].z;
    ...
    xmin = x-box.grid_width_x;
    xmax = x+box.grid_width_x;
    ymin = y-box.grid_width_y;
    ymax = y+box.grid_width_y;
    ...
    for (ii=x-box.grid_width_x;ii<=x+box.grid_width_x;ii++) {
        for (jj=y-box.grid_width_y;jj<=y+box.grid_width_y;jj++) {
            for (kk=zmin;kk<=zmax;kk++) {
                ...
                //numNeighbors += grid[m].bead_list.countLength();
                neighbor = grid[m].bead_list.head;
                 
                while(neighbor != NULL) {
                    j = neighbor->id;
                    if (i!=j) {
                        E += calc_pair_energy(i,j);
                        E += 0.5*calc_triplet_energy(i,i,j);
                        E += 0.5*calc_triplet_energy(i,j,j);
                    }
...
double Sim::calc_bead_triplet_energy(int i) {
    int ii,jj,kk,m,m2,x,y,z,j,k,iii,jjj,kkk;
    int xmin,xmax,ymin,ymax,zmin,zmax,xmin2,xmax2,ymin2,ymax2,zmin2,zmax2;
    m = beads[i].grid;
    x = grid[m].x;
    ...
    for (ii=x-box.grid_width_x;ii<=x+box.grid_width_x;ii++) {
        for (jj=y-box.grid_width_y;jj<=y+box.grid_width_y;jj++) {
            for (kk=zmin;kk<=zmax;kk++) {
                ...
                xmin2 = ii-box.grid_width_x;
                xmax2 = ii+box.grid_width_x;
                ymin2 = jj-box.grid_width_y;
                ymax2 = jj+box.grid_width_y;
                zmin2 = kk-box.grid_width_z;
                zmax2 = kk+box.grid_width_z;
                
                xmin2 = max(xmin,xmin2);
                xmax2 = min(xmax,xmax2);
                ymin2 = max(ymin,ymin2);
                ymax2 = min(ymax,ymax2);
                zmin2 = max(zmin,zmin2);
                zmax2 = min(zmax,zmax2);
                 
                while(neighbor != NULL) {
                    j = neighbor->id;
                    for (iii=xmin2;iii<=xmax2;iii++) {
                        for (jjj=ymin2;jjj<=ymax2;jjj++) {
                            for (kkk=zmin2;kkk<=zmax2;kkk++) {
                                m2 = calc_grid_placement(iii,jjj,kkk);
                                if (true) {
                                    neighbor2 = grid[m2].bead_list.head;
                                } else {
                                ...
                                while(neighbor2 != NULL) {
                                    k = neighbor2->id;
                                    if(i!=j && i!=k && j!=k) 
                                    {E += 0.5*calc_triplet_energy(i,j,k);
                                    //printf("%d\t%d\t%d\n",i,j,k);
                                    }
...
double Sim::calc_pair_energy(int i, int j) {
    ...
    E *= nus[t1][t2]*(1.0+delta) + 2.0*calc_nematic_pair_energy(i,j);
...
double Sim::calc_triplet_energy(int i, int j, int k) { //Assuming these are close enough together so that the nearest image distances are all that matter.
    double E,delta,dist,d1,d2,d3;
    int t1,t2,t3;
    LAMMPS_NS::vector r_ji,r_ki;
    t1 = beads[i].type;
    t2 = beads[j].type;
    t3 = beads[k].type;
    ...
    nearest_image_dist(r_ji,beads[j].r,beads[i].r);
    nearest_image_dist(r_ki,beads[k].r,beads[i].r);

    d1 = vec_dot(r_ji,r_ji);
    d2 = vec_dot(r_ki,r_ji);
    d3 = vec_dot(r_ki,r_ki);

    E = box.prefact3*exp(-(d1-d2+d3)/3.0*box.invsigsqr);
...
void Sim::MC_displace() { //Randomly displace all beads in the simulation
    ...
    for (int move=0;move<box.numBeads;move++) {
        ...
        //dE_test = calc_total_energy();
        //printf("PRE TRIAL: %f\t%f\n",calc_total_energy(),box.E_tot);
        ...
        //dE_test = calc_total_energy() - dE_test;
        dE = post_energy - pre_energy;
        //printf("MOVE INFO: %f\t%f\n",dE,dE_test);
        //printf("POST TRIAL: %f\t%f\n",calc_total_energy(),box.E_tot);
...
void Sim::MC_translate_molecule() { //Randomly translates an entire molecule -- however that is defined
    ...
    for (int move=0;move<box.numMolecules;move++) {
        ...
        for (j=0;j<nAtoms;j++) {
            ...
            if (flagFunc(update_chain[j].r)) noWallBreak = false;
...
void Sim::MC_rotate_molecule() { //Randomly rotate an entire molecule around its center of mass
    ...
    for (int move=0;move<box.numMolecules;move++) {
        ...
        for (i=0;i<nAtoms;i++) {
            ...
            if (flagFunc(update_chain[i].r)) noWallBreak=false;
...
void Sim::sample_TI_F() {
    int i;
    double ideal_chain_energy = 0.0;

    //Shift away from each other
    for (i=0;i<box.numBeads/2;i++) {
        beads[i].rn[2] -= 0.5*box.TI_shift;
        PBC_shift(beads[i].r,beads[i].rn);
    }

    for (i=box.numBeads/2;i<box.numBeads;i++) {
        beads[i].rn[2] += 0.5*box.TI_shift;
        PBC_shift(beads[i].r,beads[i].rn);
    ...
    box.FPlus = calc_total_energy();

    box.FPlus -= 1.5*box.numBonds;
    
    //Shift close to each other
    
    for (i=0;i<box.numBeads/2;i++) {
        beads[i].rn[2] += box.TI_shift;
        PBC_shift(beads[i].r,beads[i].rn);
    }
    ...
    box.FMinus = calc_total_energy();

    box.FMinus -= 1.5*box.numBonds;
    ...
    box.TI_dF = (box.FMinus-box.FPlus)/box.TI_shift;
...
void Sim::print_TI() {
    ...
    FILE *dump;
    dump = fopen("TI.txt","a");
    fprintf(dump,"%d\t%f\t%f\t%f\n",box.cycle,box.TI_dF,box.FMinus,box.FPlus);
    fclose(dump);
...
bool Sim::flagFunc(LAMMPS_NS::vector r) {
    LAMMPS_NS::vector perp,par,r_shift;
    int i,j;
    double dot,dist_perp,dist_par;
    for (i=0;i<box.numCNC;i++) {
        for (j=0;j<3;j++) {
            r_shift[j] = r[j] - cnc[i].R[j];
        }
        dot = vec_dot(cnc[i].U,r_shift);
        for (j=0;j<3;j++) {
            perp[j] = r_shift[j] - dot*cnc[i].U[j];
            par[j] = r_shift[j] - perp[j];
        }

        dist_perp = sqrt(vec_dot(perp,perp));
        dist_par = sqrt(vec_dot(par,par));

        if (dist_perp <= cnc[i].rad && dist_par <= 0.5*cnc[i].L) {
            return true;
        }
    }
...
void Sim::print_colors() { //Print out the PSF file. Only visualizing bonds here, not twist or angle potentials
    double min_dist = 0.16;
    int flag;
    int m,x,y,z,i,j;
    int half = box.numBeads/2;
    int ii,jj,kk,xmin,xmax,ymin,ymax,zmin,zmax; 
    double *contact_list = new double[box.numBeads];
    Bead *neighbor;
    for (ii=0;ii<box.numBeads;ii++) {
        contact_list[ii] = 0;
    }
    for (i=0;i<box.numBeads;i++) { 
        flag = 0;
        ...
        for (ii=x-box.grid_width_x;ii<=x+box.grid_width_x;ii++) {
            for (jj=y-box.grid_width_y;jj<=y+box.grid_width_y;jj++) {
                for (kk=zmin;kk<=zmax;kk++) {
                    ...
                    while(neighbor != NULL) {
                        j = neighbor->id;
                        if (beads[i].type ==0 && beads[j].type == 0) {
                            if ((i < half && j >= half) || (j < half && i>= half)) { //If neighbors are not on the same cellulose nanocrystal and are both type 0
                                if (calc_dist(beads[i].r,beads[j].r) < min_dist) {
                                    contact_list[i] = 1;
                                    neighbor = NULL;
                                    flag = 1;
                                    break;
    ...
    FILE *count_output;
    count_output = fopen("contact_count.dat","a");
    int tally = 0;
    for (i=0;i<box.numBeads;i++) {
        if (contact_list[i] == 1) tally++;
    }
    fprintf(count_output,"%d\n",tally);
    fclose(count_output);
