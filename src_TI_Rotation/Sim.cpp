...
void Sim::sample_TI_F() {
    ...
    //Rotate first CNC
    LAMMPS_NS::vector r_shift,r_rot; 

    double centerX = cnc[1].R[0];
    double centerY = cnc[1].R[1];
    double centerZ = cnc[1].R[2];
    double e1,e2,e3,e4;
    //e1 = calc_total_energy();
    for (i=box.numBeads/2;i<box.numBeads;i++) {
        r_shift[0] = beads[i].rn[0] - centerX;
        r_shift[1] = beads[i].rn[1] - centerY;
        r_shift[2] = beads[i].rn[2] - centerZ;
        
        r_rot[0] = r_shift[0];
        r_rot[1] = cos(0.5*box.TI_shift)*r_shift[1] - sin(0.5*box.TI_shift)*r_shift[2];
        r_rot[2] = sin(0.5*box.TI_shift)*r_shift[1] + cos(0.5*box.TI_shift)*r_shift[2];

        beads[i].rn[0] = centerX + r_rot[0];
        beads[i].rn[1] = centerY + r_rot[1];
        beads[i].rn[2] = centerZ + r_rot[2];
    ...
    for (i=box.numBeads/2;i<box.numBeads;i++) {
        ...
        r_rot[0] = r_shift[0];
        r_rot[1] = cos(-box.TI_shift)*r_shift[1] - sin(-box.TI_shift)*r_shift[2];
        r_rot[2] = sin(-box.TI_shift)*r_shift[1] + cos(-box.TI_shift)*r_shift[2];
    ...
    //Shift first CNC back to original position
    for (i=box.numBeads/2;i<box.numBeads;i++) {
        r_shift[0] = beads[i].rn[0] - centerX;
        r_shift[1] = beads[i].rn[1] - centerY;
        r_shift[2] = beads[i].rn[2] - centerZ;
    ...
    //e4 = calc_total_energy();
    box.TI_dF = (box.FPlus-box.FMinus)/box.TI_shift;
    //printf("%f\t%f\t%f\t%f\n",e1,e2,e3,e4);
}
