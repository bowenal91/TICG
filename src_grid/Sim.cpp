...
void Sim::addBeadToGrid(int i) { //Add a bead into its corresponding grid location
    ...
    grid[m].phi[beads[i].type] += beads[i].vol*grid[m].inc;
    if (beads[i].isRod) {
...
void Sim::MC_displace() { //Randomly displace all beads in the simulation
    ...
    Bead save;
    Grid *save_grid = new Grid[2]; //Maximum number of grids that can change are 4 since two of the beads don't actually move
...
void Sim::MC_rotate() {
    ...
    for (int move = 0;move<box.numBeads;move++) {
        ...
        post_energy += calc_grid_energy(m);
        dE = post_energy - pre_energy;
...
void Sim::writeEnergy() {
    ...
    for (i=0;i<box.numGrid;i++) {
        ...
        compression += 0.5*box.kappa*phi_tot*phi_tot*grid[i].vol*box.dens;
        ...
        chi += chi_single*grid[i].vol*box.dens;
        if (grid[i].vol ==  0.0) {
            nematic += 0.0;
        } else {
            for (j=0;j<grid[i].bead_list.size();j++) {
                for (k=j+1;k<grid[i].bead_list.size();k++) {
                    nematic += calc_nematic_pair_energy(grid[i].bead_list[j],grid[i].bead_list[k])/(box.dens*grid[i].vol);
                }
            }   
        }
    ...
    fprintf(en,"%d\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n",box.cycle,bond,angle,twist,compression,chi,nematic,bond+angle+twist+compression+chi+nematic,box.E_tot);
    fclose(en);
}
